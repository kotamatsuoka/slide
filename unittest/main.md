...
<!--
健康的とは、時間を無駄にしなかったり、精神を疲弊しなかったり、あとは朝起きて、昨日の続きの開発がしたくなるようなコードを書いていくことだったりおいて、僕はこの言葉を使っています。
...
...
-->
<br>
→ プロダクトコードを実装する前にテストコードだけレビューしてもらって、実装するものを具体的に示す！
...
<br>
→ まず、テストコードで表現する。あとはテストコードが通るようにプロダクトコードを直すだけ。
...
<br>
→ テストを書いたら実装終了ではなく、テストがあることを利用して、再設計・リファクタリングで品質を上げよう。
